...
func HandlerListUsers(s *State, cmd Command) error {
	...
	if err != nil {
		return fmt.Errorf("Something broke: %s", err.Error())
	}
...
func HandlerReset(s *State, cmd Command) error {
	...
	if err == nil {
		...
		fmt.Println(err)
		return fmt.Errorf("Deletion unsuccesful: Error %w", err)
	}
}
...
func HandlerLogin(s *State, cmd Command) error {
	...
	if err != nil {
		return fmt.Errorf("No 'user: %s' found in database, login unavailable.", argName)
	}
	s.Config.SetUser(argName)
